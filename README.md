...
**6. What's actually happening inside a call like `.invoke()`, or what a `RunnableLambda`/`RunnableParallel` does at runtime** isn't repeated in every module — it's explained once, mechanically, in two places: [module 01](modules/01_langchain_basics#execution-internals-what-invoke-actually-does) covers what `.invoke()`/`.stream()` do on a chat model (message serialization → real HTTP call → parsed back into an `AIMessage`), and [module 03](modules/03_chains_lcel#execution-internals-the-runnable-protocol) covers the `Runnable` protocol itself (what `|` builds, what `RunnableLambda`/`RunnableParallel`/`RunnablePassthrough`/`RunnableBranch` each do, and how `.batch()`/`.stream()` differ from a plain loop). Every other module that uses these constructs links back to whichever of the two is relevant instead of re-explaining them.
